...
if [[ -z $(ps -aux | grep gnome-shell | head -n 1 | awk '{ print $NF }') ]]; then
    ...
    # there's no way xdg runtime etc. are properly set if gnome-shell wasn't running;
    #  user will have to start the script after anyways, so we're bailing right now.
    exit 1
fi
